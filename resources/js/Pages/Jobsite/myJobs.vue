                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-lg-6 col-xl-6">
                                                    <h5 class="font-weight-bold">{{ job.title || 'Job Title Unavailable' }}</h5>
                                                ...
                                                <div class="col-md-6 col-lg-3 col-xl-3 border-sm-start-none border-start">
                                                    ...
                                                    <h6 class="text-success">Status: {{ job.jb_process_status }}</h6>
                                                    ...
                                                    <div class="mt-3">
                                                        ...
                                                        <p><strong>Phone:</strong> {{ job.poster_contact_number || 'N/A' }}</p>
...
export default {
    ...
    async mounted() {
        await this.fetchApplyedJobs();
    },
    methods: {
        async fetchApplyedJobs() {
            console.log("Fetching applied jobs...");
            try {
                const response = await axios.get(route("fetch_Applyed_jobs")); // Fetching job data from API
        ...
        async applyJob(jobId) {
            if (confirm("Are you sure you want to apply for this job?")) {
                try {
                    const response = await axios.post(route("apply_to_job"), { jobId });
                    alert(response.data.message); // Notify the user that the application was successful
                } catch (error) {
                    console.error("Error applying for the job:", error.response?.data || error.message);
                    alert("Failed to apply for the job. Please try again."); // Notify the user of failure
                }
            }
        },
    }
